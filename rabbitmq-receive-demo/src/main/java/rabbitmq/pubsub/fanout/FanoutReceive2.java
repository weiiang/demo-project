...
public class FanoutReceive2 {
    ...
    private static Logger logger = LoggerFactory.getLogger(FanoutReceive2.class);
    ...
    public static void receive() throws IOException {
        ...
        // 绑定队列到交换机
        channel.queueBind("receive2", "fanoutQueue", "");

        //定义消费者
        DefaultConsumer consumer = new DefaultConsumer(channel){
            @Override
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                logger.info("receive2:"+new String(body));
            }
        };

        channel.basicConsume("receive2", true, consumer);
